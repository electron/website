    ...
    [
      '@docusaurus/preset-classic',
      {
        docs: {
          ...
          // The default generator sorts `autogenerated` items by file path,
          // which orders `certificate-principal.md` before `certificate.md`
          // ('-' sorts before '.'). Sort by doc id instead so that a doc
          // comes before the docs whose names extend it.
          sidebarItemsGenerator: async ({
            ...
            return [...items].sort((a, b) =>
              sortKey(a).localeCompare(sortKey(b), 'en'),
            );
          },
          routeBasePath: '/docs/',
          editUrl: ({ docPath }) => {
